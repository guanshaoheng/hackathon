...
from graphNN_alt import Graph_HiC_Likelihood_alt
...
def train(model_name='model'):
    ...
    for epoch in range(epoch_already, epoch_already + N_EPOCHS):
        ...
        for batch_data in train_graphs:
            ...
            batch_mu, batch_std, pre_batch = model.forward(batch=batch_data)
            ...
            # loss_mse, loss = loss_cal(pre_batch, batch_data.truth, model)
            loss_mse, loss = loss_kl(batch_mu, batch_std, pre_batch, batch_data.truth, model)
            loss.backward()
            optimizer.step()
        ...
        if epoch%SHOW_PER_EPOCH==0:
            ...
            for batch_data in vali_graphs:
                with torch.no_grad():
                    ...
                    batch_mu, batch_std, pre_batch = model.forward(batch_data)
                    # loss_vali += loss_cal(torch.log(pre_batch + 1e-8), torch.log(batch_data.truth + 1e-8)).item()
                    mse_vali, loss_vali_tmp = loss_cal(pre_batch, batch_data.truth, model)
                    mse_vali, loss_vali_tmp = loss_kl(batch_mu, batch_std, pre_batch, batch_data.truth, model)
                    loss_vali += loss_vali_tmp.item()
                    print(pre_batch.sum())
                batch_num += 1
            loss_vali /= batch_num
            loss_history.append([loss_mse, mse_vali])
            print(f'Epoch [{epoch}/{N_EPOCHS + epoch_already}], Loss train: {loss_mse.item():.4e}, Loss vali: {mse_vali:.4e}')
            # print(f'Loss train large: {loss_large.item():.4e}, Loss vali large: {loss_vali_large.item():.4e}')
            # print(f'Loss train small: {loss_small.item():.4e}, Loss vali small: {loss_vali_small.item():.4e}')
            print(f'EpochTime: {(time.time() - start_epoch)/60:.2f}mins ConsumedTime: {(time.time() - start_time)/60:.2f}mins')  # 
    ...
    return epoch, loss.item(), loss_vali, loss_history
...
def loss_cal(output, target, model, lambda_=0.000):
    mse_loss = torch.nn.MSELoss()
    thres = 0.5
    mse_ = mse_loss(output, target)
    # mse_large_target = mse_loss(output[target>thres], target[target>thres])
    # mse_small_target = mse_loss(output[target<=thres], target[target<=thres])
    penalty = l2_penalty(model, lambda_)
    return mse_, mse_ + penalty

def loss_kl(mu, std, output, target, model):
    mse_loss = torch.nn.MSELoss()
    mse_ = mse_loss(output, target)
    kl_loss = model.kl_loss(mu, std)
    return mse_, mse_ + kl_loss
...
if __name__ == "__main__":
    ...
    # =============================
    #      model training
    epoch, loss, loss_vali, history = train()
