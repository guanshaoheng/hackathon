...
def train():
    ...
    for epoch in range(epoch_already, epoch_already + N_EPOCHS):
        ...
        for batch_data in train_graphs:
            ...
            # mask_selected_edges = torch.randint(0, num_edges, (NUM_RANDOM_SELECTED_EDGE_PER_EPOCH_PER_BATCH,), device=device) # 每个batch只取100个边来训练
...
def get_graph_batch(
    ...
    for sample_index in range(num_samples):
        ...
        """
         finished 确保 density和 hic 的interaction 是0-1之间的, 在restore_training_data.py中已经做了处理
        """
        x = torch.cat([
